...
def databasecreate_door(db):
    db = sql.connect("mock_database.db")
    c = db.cursor()
    c.execute("""CREATE TABLE doordata(ID TEXT, PUBLIC_KEY TEXT, SYMETRIC_KEY TEXT, IV TEXT)""")
    db.commit()
    db.close()
...
def insertdatabase_door(key, iv, client_pub):
    many_doors = 1
    db = sql.connect("mock_database.db")
    c = db.cursor()
    c.execute("INSERT INTO doordata VALUES (?, ?, ?, ?)", (str(many_doors), str(key), str(iv), str(client_pub)))
    db.commit()
...
# Convert bytes to hexadecimal string
            ...
            insertdatabase_door(key, iv, client_pub)  
            #Para ser feito... 
